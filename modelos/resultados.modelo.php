...
class ResultadosModelo{
	...
	**********************************/
	...
	{
		$stmt = Conexion::conectar()->prepare("SELECT res.id_resultados, res.orden_trabajo, res.id_normativa_analisis, res.fecha_creacion, res.usuario_creacion,nor.normativa, nor.categoria, nor.tipo_analisis, nor.analisis, res.resultado
		FROM resultados res
	...
	static public function mdlRegistrarResultados($data)
	{
		...
		try {
			...
			if ($nreg >0 ){
				...
				$stmt = Conexion::conectar()->prepare("INSERT INTO resultados(orden_trabajo,id_normativa_analisis,resultado,fecha_creacion,usuario_creacion)
				 VALUES(:orden_trabajo,:id_normativa_analisis,:resultado,:fecha_creacion,:usuario_creacion)");
